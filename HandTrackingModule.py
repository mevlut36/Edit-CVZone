...
class HandDetector:
        ...
        if  self.results.multi_hand_landmarks:
            for handType,handLms in zip(self.results.multi_handedness,self.results.multi_hand_landmarks):
                ...
                if draw:
                    ...
                    cv2.rectangle(img, (bbox[0] - 20, bbox[1] - 20), (bbox[0] + bbox[2] + 20, bbox[1] + bbox[3] + 20), (255, 205, 0), 2)
                    cv2.putText(img,myHand["type"],(bbox[0] - 30, bbox[1] - 30),cv2.FONT_HERSHEY_SIMPLEX, 1,(255, 205, 0), 2)
        if draw:
            return allHands,img
        else:
            return allHands
    ...
    def findPosition(self, img, handNo=0, draw=True):
        ...
        if self.results.multi_hand_landmarks:
            ...
            for id, lm in enumerate(myHand.landmark):
                ...
                if draw:
                    cv2.circle(img, (px, py), 3, (255,205,0), cv2.FILLED)
            xmin, xmax = min(xList), max(xList)
            ymin, ymax = min(yList), max(yList)
            boxW, boxH = xmax - xmin, ymax - ymin
            bbox = xmin, ymin, boxW, boxH
    ...
    def findDistance(self,p1, p2, img=None):
        ...
        if img is not None:
            cv2.circle(img, (x1, y1), 5, (255, 0, 100), cv2.FILLED)
            cv2.circle(img, (x2, y2), 5, (255, 0, 100), cv2.FILLED)
            cv2.line(img, (x1, y1), (x2, y2), (255, 0, 100), 2)
